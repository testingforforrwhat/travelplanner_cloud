...
public interface MenuRepository extends JpaRepository<Menu, Integer> {
  ...
  @Query(
          value = "SELECT DISTINCT m.* " +
                  ...
                  "JOIN permission_menu po ON p.permission_id = po.permission_id " +
                  "JOIN menu m ON po.menu_id = m.menu_id " +
                  "WHERE r.role_name = :userRole " +
                  "ORDER BY m.menu_id",
          nativeQuery = true
  )
  Optional<List<Menu>> findMenusByPermissionId(@Param("userRole") String userRole);
}